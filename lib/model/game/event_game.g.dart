...
abstract class _$EventGameCWProxy {
  EventGame fen(String fen);

  EventGame fullId(String? fullId);

  EventGame gameId(String gameId);

  /// This function **does support** nullification of nullable fields. All `null` values passed to `non-nullable` fields will be ignored. You can also use `EventGame(...).copyWith.fieldName(...)` to override fields one at a time with nullification support.
  ///
  /// Usage
  /// ```dart
  /// EventGame(...).copyWith(id: 12, name: "My name")
  /// ````
  EventGame call({
    String? fen,
    String? fullId,
...
/// Proxy class for `copyWith` functionality. This is a callable class and can be used as follows: `instanceOfEventGame.copyWith(...)`. Additionally contains functions for specific fields e.g. `instanceOfEventGame.copyWith.fieldName(...)`
class _$EventGameCWProxyImpl implements _$EventGameCWProxy {
  final EventGame _value;

  const _$EventGameCWProxyImpl(this._value);

  @override
  EventGame fen(String fen) => this(fen: fen);

  @override
  EventGame fullId(String? fullId) => this(fullId: fullId);

  @override
  EventGame gameId(String gameId) => this(gameId: gameId);
...
extension $EventGameCopyWith on EventGame {
  /// Returns a callable class that can be used as follows: `instanceOfclass EventGame.name.copyWith(...)` or like so:`instanceOfclass EventGame.name.copyWith.fieldName(...)`.
  _$EventGameCWProxy get copyWith => _$EventGameCWProxyImpl(this);
}
...
extension $UserGameCopyWith on UserGame {
  /// Returns a callable class that can be used as follows: `instanceOfclass UserGame extends EventGame.name.copyWith(...)` or like so:`instanceOfclass UserGame extends EventGame.name.copyWith.fieldName(...)`.
